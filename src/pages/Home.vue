...
const addToFavorite = async (item) => {
  try {
    if (!item.isFavorite) {
      ...
      const { data } = await axios.post(`https://bec253a13507dd51.mokky.dev/favorites`, obj) // Отправляем POST-запрос для добавления элемента в избранное
      ...
      await axios.delete(`https://bec253a13507dd51.mokky.dev/favorites/${item.favoriteId}`) // Отправляем DELETE-запрос для удаления элемента из избранного
...
const fetchFavorites = async () => {
  try {
    ...
    const { data: favorites } = await axios.get(`https://bec253a13507dd51.mokky.dev/favorites`)
...
const fetchItems = async () => {
  try {
    ...
    const { data } = await axios.get(`https://bec253a13507dd51.mokky.dev/items`, { params })
